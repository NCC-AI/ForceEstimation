...
class DataProcessing():
    def __init__(self, video_file, csv_file, p=True, f=True, name=False):
        ...
        self.truncation = -7
        self.df_time = self.df['Time'].as_matrix()[:self.truncation]
        self.df_target = self.df.drop(["Time"], axis=1).as_matrix()[:self.truncation]
        self.frame_id, self.frame_name = self.make_inputs()
        self.p_target, self.f_target = self.make_targets(p, f)
        self.p_dict, self.f_dict = self.make_pairs(name)

    def video_property(self):
        print('FRAME_COUNT: ',  self.frame_count)
        print('FPS: ', self.fps )
        print('TIME: ', self.frame_count/self.fps/60)
        print('SECONDS: ',self.frame_count/self.fps)
        print('FRAME_HEIGHT: ', self.frame_height)
        print('FRAME_WIDTH: ', self.frame_width)

    def csv_property(self):
        print('df keys: ', self.df.keys())
        print('df_time shape: ', self.df_time.shape)
        print('df_target shape: ', self.df_target.shape)
